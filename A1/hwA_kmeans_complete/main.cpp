...
FeaturesGrid data; // data set: features of image pixels 
unsigned k;    // number of clusters
unsigned winSize; // window size for rgb mean shift
unsigned xyWinSize; // window size for x/y mean shift
float alpha; // sparsity value to calculate the energy for AIC
...
int winSize_box; // handle for winSize-box
int xyWinSize_box; // handle for xyWinSize-box
...
void aic();
void mean_shift_cluster(); // call back for "Mean Shift" buttom
void draw(); 
...
void alpha_set(const char* s_string); // call-back function for setting parameter "alpha" (sparsity for AIC)
void winSize_set(const char* s_string); // call-back function for setting parameter "winSize" (window size for rgb mean shift)
void xyWinSize_set(const char* s_string); // call-back functino for setting parameter "xyWinSize" (window size for xy mean shift)
...
int main()
{
	...
	int button_aic = CreateButton("AIC",aic); // the last argument specifies the call-back function, see "cs1037utils.h"
	int button_meanShift = CreateButton("Mean Shift",mean_shift_cluster); // the last argument specifies the call-back function, see "cs1037utils.h"
	...
	alpha_box = CreateTextBox(to_Cstr("sparsity=" << alpha), alpha_set);
	winSize_box = CreateTextBox(to_Cstr("winSize=" << winSize), winSize_set);
	xyWinSize_box = CreateTextBox(to_Cstr("xyWinSize=" << xyWinSize), xyWinSize_set);
	...
	DeleteControl(button_aic);
	DeleteControl(button_meanShift);
	...
	DeleteControl(winSize_box);
	DeleteControl(xyWinSize_box);
...
void mode_set(int index)
{
	...
	::data.reset(image,(FEATURE_TYPE) mode,w);
	draw();
}
...
void image_load(int index) 
{
	...
	int height = max(100,(int)image.getHeight())+ 2*pad + 2*cp_height;
    ...
    SetControlPosition(    alpha_box, image.getWidth()+pad+5, cp_height+pad+90);
    SetControlPosition(    winSize_box,image.getWidth()+pad+5, cp_height+pad+120);
	SetControlPosition(xyWinSize_box, image.getWidth() + pad + 5, cp_height + pad + 150);
    SetControlPosition(check_box_view,image.getWidth()+pad+5, cp_height+pad+180);
	...
	SetTextBoxString(alpha_box, to_Cstr("sparsity=30000000"));
	SetTextBoxString(winSize_box, to_Cstr("winSize=100"));
	SetTextBoxString(xyWinSize_box, to_Cstr("xyWinSize=200"));
	::data.reset(image,(FEATURE_TYPE) mode,w);
	draw();
}
...
void cluster()
{
	cout << " number of iterations is " << ::data.clusterAlgo(true, k) << endl;
	draw();
}

void aic()
{
	unsigned best_k = ::data.AIC(alpha);
	cout << "best k is " << best_k << endl;
	k_set(to_Cstr("k=" << best_k));
	draw();
}

void mean_shift_cluster()
{
	//TODO: windowsize, not k
	int meanshift_k = ::data.clusterAlgo(false, k, winSize, xyWinSize);
	cout << " number of iterations is " << meanshift_k << endl;
	k_set(to_Cstr("k=" << meanshift_k));
	draw();
}
...
void k_set(const char* k_string) {
	...
	if (temp_k>1 && temp_k!=k) {k=(unsigned)temp_k; cout << "parameter 'k' is set to " << k << endl;}
...
void w_set(const char* w_string) {
	...
	if (temp_w>=0.0 && temp_w!=w) {w=temp_w; cout << "parameter 'w' is set to " << w << endl; ::data.reset(image,(FEATURE_TYPE) 

mode,w);}
	SetTextBoxString(w_box,to_Cstr("w=" << w));
}

// call-back function for setting parameter alpha (sparsity for AIC)
void alpha_set(const char* alpha_string) {
	float temp_alpha;
	sscanf_s(alpha_string, "sparsity=%f", &temp_alpha);
	if (temp_alpha>=0.0 && temp_alpha!=alpha) {alpha=temp_alpha; cout << "parameter 'sparsity' is set to " << alpha << endl;}
	else cout << "parameter 'sparsity' should be greater than 0, 'sparsity' remains " << alpha << endl;
	SetTextBoxString(alpha_box, to_Cstr("sparsity=" << alpha));
}

// call-back function for setting parameter winSize (window size for mean shift)
void winSize_set(const char* alpha_string) {
	unsigned temp_winSize;
	sscanf_s(alpha_string, "winSize=%d", &temp_winSize);
	if (temp_winSize>=0 && temp_winSize!=winSize) {winSize=temp_winSize; cout << "parameter 'winSize' is set to " << winSize << 

endl;}
	else cout << "parameter 'winSize' should be greater than 0, 'winSize' remains " << winSize << endl;
	SetTextBoxString(alpha_box, to_Cstr("winSize=" << winSize));
}

void xyWinSize_set(const char* gamma_string) {
	unsigned temp_xyWinSize;
	sscanf_s(gamma_string, "xyWinSize=%d", &temp_xyWinSize);
	if (temp_xyWinSize >= 0 && temp_xyWinSize != xyWinSize) { xyWinSize = temp_xyWinSize; cout << "parameter 'xyWinSize' is set to " << temp_xyWinSize << endl; }
	else cout << "parameter 'xyWinSize' should be greater than 0, 'xyWinSize' remains " << xyWinSize << endl;
	SetTextBoxString(alpha_box, to_Cstr("xyWinSize=" << xyWinSize));
}
...
void brush_real(Point click, int drag)
{
	...
	if      ((drag==1 || drag==2) && c>='0' && c<='9') {
		...
		if (t<k) for (i=start; i<end; ++i) ::data.addSeed(click+(*i),t); // function addSeed() is defined in FeaturesGrid.h
	}
	else if (drag==3) for (i=start; i<end; ++i) ::data.addSeed(click+(*i),NO_LABEL);
}
...
{ 
	...
	const vector<vector<double>> means = ::data.getMeans();
	if (view && !means.empty()) {
		for (unsigned t=0; t<k; t++) {
			transparancy1[t] = 1.0;
			colors[t]        = RGB((unsigned char) means[t][0],(unsigned char) means[t][1], (unsigned char) means[t]
...
[2]);
	...
	if (!::data.getLabeling().isEmpty()) drawImage(::data.getLabeling(),colors,transparancy1); // 4th draw() function in Image2D.h
	if (!::data.getSeeds().isEmpty())    drawImage(::data.getSeeds(),colors,transparancy2); // 4th draw() function in Image2D.h
}
...
void image_saveA() 
{
	...
	if (mode==rgb || mode ==rgbxy) {    // SAVES REGION MASK OVER THE ORIGINAL IMAGE (similar to what is drawn on the window)
		RGB    colors[13]        = { red,  blue, green, cyan, yellow, magenta, teal, navy, lime, maroon, purple, olive, 
...
black};
		...
		Table2D<RGB>    cMat1 = convert<RGB>(::data.getLabeling(),Palette(colors));   //3rd convert method in Table2D.h
		Table2D<double> aMat1 = convert<double>(::data.getLabeling(),Palette(transparancy1)); 
		tmp = cMat1%aMat1 + image%(1-aMat1);  // painting labeling over image
		Table2D<RGB>    cMat2 = convert<RGB>(::data.getSeeds(),Palette(colors));   //3rd convert method in Table2D.h
		Table2D<double> aMat2 = convert<double>(::data.getSeeds(),Palette(transparancy2)); 
...
[2]);
		...
		Table2D<RGB>    cMat = convert<RGB>(::data.getLabeling(),Palette(colors));   //3rd convert method in Table2D.h
		Table2D<double> aMat = convert<double>(::data.getLabeling(),Palette(transparancy)); 
